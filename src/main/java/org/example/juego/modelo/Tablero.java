...
public class Tablero {
    ...
    private Map<Jugador, Pane> fichaPorJugador = new HashMap<>();

    public Tablero() {
    }


    public void inicializarTablero(GridPane tablero) {
        int filas = tablero.getRowCount();
        int cols  = tablero.getColumnCount();
        int filaCentral = filas / 2;
        int radio = filaCentral;

        // Crear casillas y Panes con estilos originales
        for (int fila = 0; fila < filas; fila++) {
            for (int columna = 0; columna < cols; columna++) {
                ...
                Pane celda = new Pane();
                celda.setOpacity(1.0);                    // o 0.3 si quieres semitransparente
                celda.setUserData(celda.getOpacity());    // guardamos la opacidad original
...
//                        celda.setStyle("-fx-background-color: yellow; -fx-background-radius:5;-fx-border-width: 2;");
                    asignarColores(celda, casilla);
                } else if (isCircunferencia(fila, columna, filaCentral, radio)) {
//                    celda.setStyle("-fx-background-color: orange; -fx-background-radius:5;-fx-border-width: 2;");
                    asignarColores(celda, casilla);
                }else{
                    celda.setStyle("-fx-background-color: white; -fx-background-radius:3; -fx-border-width: 2; -fx-border-color: #e1e1e1;");
                }

                // esquinas especiales que no quiero que aparezcan en mi tablero
                if (isEsquinaEspecial(fila, columna, filas)) {
                    casilla.setTipo("VACIO");
                    casilla.setVisible(false);                   // <-- marca invisible
                    celda.setStyle("-fx-background-color: white; -fx-background-radius:3; -fx-border-width: 2; -fx-border-color: #e1e1e1;");
                }
        ...
        // Ficha visual en el centro
        fichaVisual = new Pane();
        fichaVisual.setStyle("-fx-background-color: red; -fx-background-radius:100%; -fx-pref-width:20; -fx-pref-height:20;");
        tablero.add(fichaVisual, currentCol, currentRow);
        GridPane.setHalignment(fichaVisual, HPos.CENTER);
        GridPane.setValignment(fichaVisual, VPos.CENTER);
    }
    ...
    public void moverFicha(GridPane tablero, Casilla destino, Jugador jugadorTurno) {
        // solo quita la ficha, no el tablero
        tablero.getChildren().remove(fichaVisual);
        currentRow = destino.getX();
        currentCol = destino.getY();
        tablero.add(fichaVisual, currentCol, currentRow);
        GridPane.setHalignment(fichaVisual, HPos.CENTER);
        GridPane.setValignment(fichaVisual, VPos.CENTER);

        jugadorTurno.setFilaActual(currentRow);
        jugadorTurno.setColumnaActual(currentCol);
    }
